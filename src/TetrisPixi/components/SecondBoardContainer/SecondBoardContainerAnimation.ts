...
export function scoreAnimationTween(scoreText: PIXI.Text): TWEEN.Tween<scoreAnimationTweenType> {
    ...
    let scoreAnimationIn = new TWEEN.Tween(scoreTextValues)
        .to({x: 1.4, y: 1.4, rotation: -0.2}, 100)
        .onUpdate(() => {
            onValuesUpdate(scoreText, scoreTextValues)
        });

    let scoreAnimationOut = new TWEEN.Tween(scoreTextValues)
...
export function placedTetriminosTween(boardContainer: PIXI.Container, initialPosition: number, offset: number): TWEEN.Tween<placedTetriminosTweenType> {
    let position: placedTetriminosTweenType = {
        y: initialPosition,
    };

    let placedAnimationIn = new TWEEN.Tween(position)
        .to({y: initialPosition + offset}, 100)
        .onUpdate(() => {
