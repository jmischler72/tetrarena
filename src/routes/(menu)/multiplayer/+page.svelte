<script lang="ts">
    ...
    import MenuButtonHeader from "$lib/components/menu-components/MenuButtonHeader.svelte";
...
<MenuContainer>
    <MenuHeader slot="header">
        <MenuButtonHeader on:click={()=> currentMenu = 'list'}
                    menu="{{text: 'Rooms List', icon: 'list', selected: currentMenu === 'list'}}"></MenuButtonHeader>
        <MenuButtonHeader on:click={()=> currentMenu = 'create'}
                    menu="{{text: 'Create Room', icon: 'add_circle', selected: currentMenu === 'create'}}"></MenuButtonHeader>
    </MenuHeader>
