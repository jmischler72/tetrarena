			...
			it('should authenticates user with valid credentials and provides a jwt token', () => {
				return request(app.getHttpServer())
					...
					.then(({ body }) => {
						...
						expect(refreshTokenJwt).toMatch(/^[A-Za-z0-9-_=]+\.[A-Za-z0-9-_=]+\.?[A-Za-z0-9-_.+/=]*$/);
			...
			it('should update one user', async () => {
				return await request(app.getHttpServer())
					...
					.send({
						...
						username: 'username #1',
						email: 'test@example.com',
						password: '$2b$10$hgJzgGh2tkqqIYpIYQI9pO0Q1S9Vd.OXnJcsm1oA1nYvd9yet8sxi',
					})
					.expect(HttpStatus.OK)
					.then(({ body }) => {
						expect(body).toEqual({
		...
		describe('Delete on user [DELETE /api/users/:id]', () => {
			it('should delete one user by id', async () => {
				return await request(app.getHttpServer())
					...
					.expect(HttpStatus.OK)
					.then(() => {
						return request(app.getHttpServer()).get('/users/1').expect(HttpStatus.NOT_FOUND);
					});
			});
