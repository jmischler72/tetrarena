		...
		it('should throw an error for a bad name', async () => {
			return await request(app.getHttpServer())
				...
				.then(({ body }) => {
					expect(body).toEqual({
						error: 'Bad Request',
						message: ['name must be shorter than or equal to 30 characters', 'name must be a string'],
						statusCode: 400,
					});
					expect(new BadRequestException());
				});
		...
		it('should throw an error for a bad username', async () => {
			return await request(app.getHttpServer())
				...
				.then(({ body }) => {
					expect(body).toEqual({
						error: 'Bad Request',
						message: ['username must be shorter than or equal to 40 characters', 'username must be a string'],
						statusCode: 400,
					});
					expect(HttpStatus.BAD_REQUEST);
					expect(new BadRequestException());
